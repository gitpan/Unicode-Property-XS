...
our @scripts = (
         ...
         'Braille', 'Buginese', 'Buhid', 'CanadianAboriginal',
         'Cherokee', 'Coptic', 'Cypriot', 'Cyrillic',
         'Deseret', 'Devanagari', 'Ethiopic', 'Georgian',
         ...
         'Latin', 'Limbu', 'LinearB', 'Malayalam',
         'Mongolian', 'Myanmar', 'NewTaiLue', 'Ogham',
         'OldItalic', 'OldPersian', 'Oriya', 'Osmanya',
         'Runic', 'Shavian', 'Sinhala', 'SylotiNagri',
         'Syriac', 'Tagalog', 'Tagbanwa', 'TaiLe',
...
Functions for bidirectional properties:
    C<ucs_BidiL()>, C<ucs_BidiLRE()>, C<ucs_BidiLRO()>, C<ucs_BidiR()>, C<ucs_BidiAL()>, 
    C<ucs_BidiRLE()>, C<ucs_BidiRLO()>, C<ucs_BidiPDF()>, C<ucs_BidiEN()>, C<ucs_BidiES()>, 
    C<ucs_BidiET()>, C<ucs_BidiAN()>, C<ucs_BidiCS()>, C<ucs_BidiNSM()>, C<ucs_BidiBN()>, 
    C<ucs_BidiB()>, C<ucs_BidiS()>, C<ucs_BidiWS()>, C<ucs_BidiON()>.

Functions for scripts ( properties Balinese, Cuneiform, Nko, PhagsPa, Phoenician, are not included 
since they are not implemented in /\p{ }/ form. ):
    C<ucs_Arabic()>, C<ucs_Armenian()>, C<ucs_Bengali()>, C<ucs_Bopomofo()>, C<ucs_Braille()>,
    C<ucs_Buginese()>, C<ucs_Buhid()>, C<ucs_CanadianAboriginal()>, C<ucs_Cherokee()>,
    C<ucs_Coptic()>, C<ucs_Cypriot()>, C<ucs_Cyrillic()>, C<ucs_Deseret()>, C<ucs_Devanagari()>,
    C<ucs_Ethiopic()>, C<ucs_Georgian()>, C<ucs_Glagolitic()>, C<ucs_Gothic()>, C<ucs_Greek()>,
    C<ucs_Gujarati()>, C<ucs_Gurmukhi()>, C<ucs_Han()>, C<ucs_Hangul()>, C<ucs_Hanunoo()>,
    C<ucs_Hebrew()>, C<ucs_Hiragana()>, C<ucs_Inherited()>, C<ucs_Kannada()>, C<ucs_Katakana()>,
    C<ucs_Kharoshthi()>, C<ucs_Khmer()>, C<ucs_Lao()>, C<ucs_Latin()>, C<ucs_Limbu()>,
    C<ucs_LinearB()>, C<ucs_Malayalam()>, C<ucs_Mongolian()>, C<ucs_Myanmar()>, C<ucs_NewTaiLue()>,
    C<ucs_Ogham()>, C<ucs_OldItalic()>, C<ucs_OldPersian()>, C<ucs_Oriya()>, C<ucs_Osmanya()>,
    C<ucs_Runic()>, C<ucs_Shavian()>, C<ucs_Sinhala()>, C<ucs_SylotiNagri()>, C<ucs_Syriac()>,
    C<ucs_Tagalog()>, C<ucs_Tagbanwa()>, C<ucs_TaiLe()>, C<ucs_Tamil()>, C<ucs_Telugu()>,
    C<ucs_Thaana()>, C<ucs_Thai()>, C<ucs_Tibetan()>, C<ucs_Tifinagh()>, C<ucs_Ugaritic()>,
    C<ucs_Yi()>. 

Functions for derived properties:
        C<ucs_Alphabetic()>, C<ucs_Lowercase()>, C<ucs_Uppercase()>, C<ucs_Math()>,
        C<ucs_IDStart()>, C<ucs_IDContinue()>, C<ucs_Any()>, C<ucs_Assigned()>,
        C<ucs_Unassigned()>, C<ucs_ASCII()>, C<ucs_Common()> ); 
Functions for extended properties:
        C<ucs_ASCIIHexDigit()>, C<ucs_BidiControl()>, C<ucs_Dash()>, C<ucs_Deprecated()>,
        C<ucs_Diacritic()>, C<ucs_Extender()>, C<ucs_HexDigit()>, C<ucs_Hyphen()>,
        C<ucs_Ideographic()>, C<ucs_IDSBinaryOperator()>, C<ucs_IDSTrinaryOperator()>, C<ucs_JoinControl()>,
        C<ucs_LogicalOrderException()>, C<ucs_NoncharacterCodePoint()>, C<ucs_OtherAlphabetic()>, C<ucs_OtherDefaultIgnorableCodePoint()>,
        C<ucs_OtherGraphemeExtend()>, C<ucs_OtherIDStart()>, C<ucs_OtherIDContinue()>, C<ucs_OtherLowercase()>,
        C<ucs_OtherMath()>, C<ucs_OtherUppercase()>, C<ucs_PatternSyntax()>, C<ucs_PatternWhiteSpace()>,
        C<ucs_QuotationMark()>, C<ucs_Radical()>, C<ucs_SoftDotted()>, C<ucs_STerm()>,
        C<ucs_TerminalPunctuation()>, C<ucs_UnifiedIdeograph()>, C<ucs_VariationSelector()>, C<ucs_WhiteSpace()> ); 
...
Copyright (C) 2008 by Mindos Cheng
